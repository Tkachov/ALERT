smpcmod_importer = {
	...
	render: function () {
		...
		var e = document.getElementById("smpcmod_importer");
		e.innerHTML = "";
		if (closable)
		...
		if (this.form_dom == null) {
			...
			var h = createElementWithTextNode("b", "Import .smpcmod into stage");
			f.appendChild(h);
			
			var file_input = document.createElement("input");
			file_input.type = "file";
			file_input.name = "smpcmod";
			...
			stage_autocomplete.id = "smpcmod_importer_stage_autocomplete";
			...
			file_input.onchange = function () {
				if (stage_input.value == "" && file_input.files.length > 0)
					stage_input.value = file_input.files[0].name.replace(".smpcmod", "").replace(".mmpcmod", "");
			};
			...
			b.onclick = function () {
				...
				self.import_smpcmod(file_input, stage_input.value);
			};
		}
	...
	import_smpcmod: function (file_input, stage) {
		this.sending = true;
		...
		ajax.postFormAndParseJson(
			"api/stages/import_smpcmod", form_data,
			function(r) {
				self.sending = false;
	...
	_show: function (e) {
		e = e || document.getElementById("smpcmod_importer");
